...
* **item 10 (RString retirement) pilot #1, 2026-09-12.** Migrated `Grade.cpp`/`Grade.h` (10 total RString mentions, the smallest leaf subsystem in the codebase) from RString to std::string, per `playbooks/migrate-rstring.md`. `GradeToString`/`GradeToOldString`/`GradeToLocalizedString` return std::string now; `StringToGrade` takes `const std::string&`. Confirmed the playbook's boundary-safety claim holds with zero caller-side changes across 8 caller files, backed by StdString.h:361's `CStdStr(const std::string&)` converting constructor plus CStdString's inheritance from std::basic_string<char>. One gotcha: `.MakeUpper()` is a CStdString-only method - replaced with the RageUtil free function guarded the same way CStdStr::MakeUpper() itself does internally. Verified: sm_tests 5966/226 unchanged, [Grade] tag 38/4 unchanged, ctest 100%, Release StepMania-R.exe clean rebuild, --SelfTest exit 0. Playbook history updated with the known-good pattern for picking the next subsystem.
* **item 10 (RString retirement) pilot #2, 2026-09-12.** Migrated `Command.cpp`/`Command.h` (~20 RString mentions) per `playbooks/migrate-rstring.md`. Migrated only the scalar boundary values (`Command::GetName()`, `Command::GetOriginalCommandString()`, `Commands::GetOriginalCommandString()`, `Command::Arg::s`) to std::string; deliberately kept internal storage (`m_vsArgs`, `Load()`/`ParseCommands()` params, the split()/join() calls) as RString, since RageUtil's split/join take std::vector<RString> and container element types don't inherit the base/derived boundary safety (new "containers don't inherit boundary safety" gotcha recorded in the playbook). Found one real hard-boundary blocker: Difficulty.h's `StringToDifficulty(const RString&)` takes its argument by reference to the derived type, so a plain std::string argument can't bind to it - fixed with 2 explicit `RString(...)` wraps in UnlockManager.cpp's call sites. All other boundary calls (StringToInt already const std::string&, by-value RString params, RString variable assignment, .c_str()) needed no changes. Verified: sm_tests 5966/226 unchanged, [Command] tag 26/6 unchanged, ctest 100%, Release StepMania-R.exe clean rebuild, --SelfTest exit 0. Playbook gotchas section updated with both new findings (hard-reference-boundary direction, container non-covariance).
* **item 10 (RString retirement) pilot #3, 2026-09-12.** Migrated `ScoreDisplayCalories.cpp`/`.h`'s single RString member (`m_sMessageOnStep`) to std::string. Trivial, fully self-contained pilot - the MessageManager::Unsubscribe(const RString&) hard-boundary wrap landed in the same file. Found the member is declared/checked but never actually assigned anywhere in the class (stays default-empty for every instance's lifetime, so the Unsubscribe branch never runs in practice) - pre-existing dead-ish state, unrelated to this migration, left as-is. Scouted and explicitly rejected MeterDisplay.cpp/ComboGraph.cpp (both forward a by-value RString param straight into AutoActor/ThemeManager/ThemeMetric APIs - large un-migrated subsystems, no real footprint reduction) and PlayerAI.cpp (feeds into IniFile/XNode, already named in the playbook's "heavy RString users" warning). New lesson recorded in the playbook: RString mention count alone isn't a sufficient pilot signal - check whether each mention stays local or passes straight through to a big subsystem's reference-taking API. Verified: sm_tests 5966/226 unchanged, ctest 100%, Release StepMania-R.exe clean rebuild, --SelfTest exit 0.
* **modernization status check, 2026-09-13: non-blocking backlog confirmed exhausted for now.** Re-scouted ~15 more small item-10 (RString) candidate files (Bookkeeper, CourseContentsList, LuaExpressionTransform, ModsGroup, ScrollBar, ActiveAttackList, BeginnerHelper, CryptHelpers, GradeDisplay, OptionsCursor, DancingCharacters) file-by-file rather than by grep count alone. Every one either passes its RString straight into a large un-migrated subsystem, or hits a genuine hard boundary not safe to paper over (LuaExpressionTransform.cpp's `error` local feeds RunScriptOnStack's non-const-reference out-param - a temporary-copy wrap would silently discard the callee's write-back, a real behavior risk). Item 10's opportunistic small-file lane is exhausted for now; 3 pilots (Grade, Command, ScoreDisplayCalories) stand from this session. Also confirmed items 13 and 16's stated remainders are non-actionable (13 has no reproducer; 16's remainder is macOS/Linux work explicitly gated behind AGENTS.md §3's Windows-first priority). Item 9 (god-object splitting) is now excluded for a concrete reason, not just general risk-aversion: its own playbook's verification step requires live manual gameplay spot-checks an autonomous session cannot perform. Conclusion: there is currently no more genuinely safe, autonomous, non-blocking backlog work available beyond what's already gated. Recorded in memory and DocsAgents/modernization-backlog.md so a future "complete the backlog" pass doesn't re-derive this from scratch.
* **item 9 (god-object split) phase 1, cluster 1, 2026-09-13.** Maintainer directed this specific cluster: carved GameState's "Edit stuff" (8 members + GetEditLocalProfile()) out into a new GameStateEditData.h/.cpp, registered in CMakeData-singletons.cmake. Achieved genuine zero call-site churn for raw public data members (not just accessor methods) via reference members in GameState.h bound to the new m_EditData component in the constructor init-list — ~112 sites across 13 files (ArrowEffects/NoteDisplay/NoteField/OptionRowHandler/Player/PlayerState/ScreenEdit/ScreenOptionsEditProfile/ScreenOptionsManageCourses/ScreenOptionsManageProfiles/Steps.cpp plus GameState.cpp's own Luna binding block) needed zero changes. Safe because m_EditData is a same-lifetime value member (GameState's copy ctor/assignment are already deleted) declared before the reference members. Reset parity automatic since GameState::Reset()'s existing lines weren't touched. Verified: sm_tests unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0.
* **Pay mode investigated, 2026-09-13 — confirmed already fully functional, nothing to reactivate.** Maintainer asked to reactivate Pay mode (Home/Free/Pay), suspecting it had been disabled. Found CoinMode_Pay is fully wired (credits math, System Options 3-way selector, credits overlay, debug-overlay cycle). A real upstream removal saga existed once but every commit in it (including the restoration) predates this fork. The actual reason Pay looks inert: PrefsManager's EventMode preference defaults true (git blame: 2011-03-17, genuine upstream default, not fork-specific), and GameState::GetCoinMode() silently downgrades Pay to Free whenever EventMode is on - a separate preference not exposed in the same selector. Confirmed and pinned with a new characterization test, tests/test_GameState.cpp ([GameState][CoinMode] tag, registered in tests/CMakeLists.txt), covering Home/Free/Pay-with-EventMode-on(downgrades)/Pay-with-EventMode-off(charges correctly). No production code changed - maintainer asked only to confirm, not change the default.
* **item 9 (god-object split) phase 1, cluster 2, 2026-09-13.** Carved GameState's "used in workout" fields (m_bGoalComplete[NUM_PLAYERS], m_bWorkoutGoalComplete, GetGoalPercentComplete()) into GameStateWorkoutData.h/.cpp, registered in CMakeData-singletons.cmake. Even smaller blast radius than cluster 1 - only 3 files touch this cluster (GameState.h/.cpp, ScreenGameplay.cpp). Same reference-member zero-call-site-churn technique as cluster 1, with a new wrinkle: m_bGoalComplete is an array, so its reference member uses the T (&name)[N] declarator (reference-to-array, not array-of-references, which doesn't exist). GetGoalPercentComplete()'s full body moved as a real implementation (not just a forward) since it only touched PROFILEMAN/STATSMAN + its own parameter, no other GameState state. IsGoalComplete()/GetGoalPercentComplete() stay thin inline forwards on GameState. No new test added (pure mechanical relocation, identical behavior, covered by the existing suite). Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0.
* **item 9 (god-object split) phase 1, cluster 3, 2026-09-13.** Carved GameState's "Attract stuff" (m_iNumTimesThroughAttract, IsTimeToPlayAttractSounds(), VisitAttractScreen()) into GameStateAttractData.h/.cpp, registered in CMakeData-singletons.cmake. Scouted "Award stuff" first and rejected it: its StageAward/PeakComboAward types are used broadly across the scoring pipeline (8 files: HighScore.cpp/.h, PlayerStageStats.cpp/.h, StageStats.cpp, etc.), a much bigger real blast radius than the cluster's small in-GameState footprint suggested - noted as a lesson: check every candidate's full footprint (types included), not just the god object's own members. Attract stuff itself only touches 4 files (GameState.h/.cpp, ScreenAttract.cpp, ScreenDemonstration.cpp), both methods moved as real implementations since they only touched PREFSMAN/CommonMetrics, no other GameState state. Plain int reference member, no new wrinkle. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0.
* **item 9 (god-object split) phase 1, cluster 4, 2026-09-13 (autonomous, "completa el backlog solo con lo no bloqueante" goal).** Carved GameState's "Autogen stuff" (m_autogen_fargs, GetAutoGenFarg()) into a header-only GameStateAutogenData.h - no .cpp needed since the whole class is one inline getter + one vector member, matching how it already lived inline in GameState.h. Smallest cluster yet: only GameState.h/.cpp and one external read (NoteDataUtil.cpp:837) touch it. The original author's own comment already flagged this as should-be-its-own-thing. GameState.cpp's Luna<GameState> Lua-binding thunks (GetAutoGenFarg/SetAutoGenFarg) call container methods directly on the moved member (push_back/size/operator[]) - all kept working unchanged via the reference member. Registered the header in CMakeData-singletons.cmake. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0.
* **item 9 (god-object split) phase 1, cluster 5, 2026-09-13 (autonomous, standing goal).** Carved GameState's "Haste" fields (m_fHasteRate, m_fLastHasteUpdateMusicSeconds, m_fAccumulatedHasteSeconds - 3 plain floats, no associated methods at all) into a header-only GameStateHasteData.h, registered in CMakeData-singletons.cmake. All 13 ScreenGameplay.cpp touches checked individually before committing to this cluster (per the "verify full footprint" lesson from rejecting Award Stuff) - confirmed all are simple direct reads/writes/CLAMP()/SCALE() calls, nothing hidden. Important correctness check: none of the three fields were previously in GameState's constructor init-list (left implicitly uninitialized until the first Reset() call), so GameStateHasteData deliberately has NO explicit constructor - adding one that zero-initializes would have silently changed behavior versus the original. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 5 clusters done now (Edit, Workout, Attract, Autogen, Haste).
* **item 10 (RString retirement) pilot #4, 2026-09-13 (autonomous, standing goal).** Migrated StyleUtil.h/.cpp's StyleID class (sGame/sStyle, both private members - found by widening the scouting net to 2-3-mention files and specifically looking for private members, which have zero external call-site exposure by construction). Found a new hard-boundary shape: XNode::GetAttrValue(const RString&, T &out) is templated on T so it looks generic, but forwards to XNodeValue::GetValue(T &out) which is NOT templated - only 5 concrete virtual overloads exist (RString&/int&/float&/bool&/unsigned&), so T=std::string fails to resolve. A templated wrapper isn't proof the underlying call is generic - new lesson recorded, watch for this with any XNodeValue-backed API (IniFile, XmlFile). Fixed with a local RString temporary on the read side in StyleID::LoadFromNode(); the write side (AppendAttr, by-value in its own template, feeding SetValue(const RString&)) needed no change. Also confirmed the earlier finding that GameManager.cpp is not a valid item-9 target (stateless, no data members - it's item 20's territory, excluded) and Profile.cpp was scouted and rejected for item 9 (serialization-coupled: single giant constructor spanning nearly all fields, three separate places - constructor, MergeScoresFromOtherProfile, SwapExceptPriority - that must stay in sync per the class's own code comments, matching the playbook's own "serialization" risk warning). Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0.
* **item 10 (RString retirement) pilot #5, 2026-09-13 (autonomous, standing goal).** Migrated CourseUtil.h/.cpp's CourseID class (sPath, sFullTitle - private members, same scouting heuristic as pilot 4). Found a new wrinkle: a private member can still have external exposure through a public getter returning a reference to it - GetPath() previously returned const RString&, which can't reference a std::string member once migrated. Fixed by dropping the reference (return by value instead) rather than changing the return type, since it's called from only one place (Profile.cpp:2144 via splitpath()) and a by-value RString return binds to that const-ref caller with zero external changes - cleaner than re-checking every caller for the const-std::string&-return hard-boundary issue. Replaced two .Left(n) calls with .substr(0,n) per the playbook's existing mapping table, confirmed exact via StdString.h:500 (CStdStr::Left is itself a clamped substr(0,n)). Hit the same XNode::GetAttrValue hard-boundary shape as pilot 4 in LoadFromNode(), same fix (local RString temporaries). CourseID is section-5-adjacent (.crs course-file identity) - re-verified [crs] tag (39/5) unchanged before/after. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0.
* **item 10 (RString retirement) pilot #6, 2026-09-13 (autonomous, standing goal).** Migrated SongUtil.h/.cpp's SongID class (sDir, private, no exposing getter). Checked the whole family of similarly-shaped ID classes together (SongID/StepsID/TrailID/CourseID/StyleID) - found TrailID has zero RString members at all (nothing to migrate), and StepsID's sDescription is private with no exposing getter (clean future pilot, not done this round). SongID::sDir was identically shaped to CourseID::sPath: same two .Left(n)->.substr(0,n) replacements, same XNode::GetAttrValue hard-boundary fix via a local RString temporary in LoadFromNode(). Song::GetSongDir()/SongManager::GetSongFromDir(RString) both qualify as boundary-safe under the established rules. SongID is section-5-adjacent (core song identity/loading) - re-verified [corpus] tag (313/3) unchanged before/after. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0.
* **item 10 (RString retirement) pilot #7, 2026-09-13 (autonomous, standing goal).** Migrated StepsUtil.h/.cpp's StepsID::sDescription, closing out this round's *ID family scout (SongID/StepsID/TrailID/CourseID/StyleID). Two hard boundaries this time: the usual XNode::GetAttrValue shape in LoadFromNode() (local RString temporary), plus a new one - SongUtil::GetOneSteps(...)'s sDescription parameter is const RString& (a real reference parameter, unlike the by-value RString parameters every earlier *ID pilot happened to hit) - fixed with an explicit RString(sDescription) wrap at the one call site in ToSteps(). Steps::GetDescription() returns RString by value (safe), AppendAttr's by-value template param (safe), operator</operator=='s COMP() macros use native std::string comparisons (safe). Section-5-adjacent (Steps/chart identity) - re-verified [corpus] tag (313/3) unchanged before/after. New lesson recorded: by-value vs by-reference isn't predictable from prior pilots' pattern - check every call's actual signature every time. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0.
* **item 10 (RString retirement) pilot #8, 2026-09-13 (autonomous, standing goal).** Migrated CryptHelpers.h/.cpp's RSAKeyWrapper::Load(...) - a new shape: migrating a public function's own parameter types (const RString &sKey, RString &sError), not an internal member behind an unchanged interface. Usually the bigger, more externally-visible kind of change, but stayed small: only 4 call sites, all in CryptManager.cpp. Changed to (const std::string&, std::string&) directly - every existing RString argument/out-param variable at the call sites bound with zero changes needed, since RString's inheritance from std::string makes a derived-typed variable satisfy a base-typed reference parameter automatically. Confirmed the blast radius really was contained: the rebuild only recompiled 2 files (CryptHelpers.cpp, CryptManager.cpp), no wider header-inclusion cascade. Load()'s body only used sKey.data()/.size() (native) and assigned error_to_string(iRet)'s result into sError (safe either direction). Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0.
* **item 10 (RString retirement) pilot #9, 2026-09-13 (autonomous, standing goal).** Migrated ScoreKeeper.h/.cpp's MakeScoreKeeper(...) factory method - a by-value parameter version of pilot 8's shape (RString sClassName -> std::string sClassName), simpler still since a by-value param accepts either RString or std::string caller argument unambiguously. Only 1 external caller (ScreenGameplay.cpp:169). Body only compares sClassName=="..." (native) and calls .c_str() (native) - zero hard boundaries. Also scouted Trail.h's public Modifiers member and deliberately did not migrate it: it's touched from 3 external files (Course.cpp, CourseContentsList.cpp, GameState.cpp), one call (PlayerOptions::FromString) takes a real const RString& reference param (hard boundary, needs a wrap), and Message::SetParam(const RString&, const T&) is a template forwarding to LuaHelpers::Push(L, val) - possibly the same templated-wrapper-non-template-backing trap found with XNode::GetAttrValue, not yet confirmed. Flagged as a real future candidate once LuaHelpers::Push's overload set is checked, not attempted half-sure. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0.
* **item 10 (RString retirement) pilot #10, 2026-09-13 (autonomous, standing goal).** Migrated TrailEntry::Modifiers (Trail.h/.cpp), unblocking the candidate flagged in pilot 9. Checked LuaHelpers::Push's overload set (LuaManager.cpp:93) and found a genuine std::string template specialization already exists alongside the RString one - unlike XNode::GetAttrValue, this template really is generic, so Message::SetParam calling it needed no wrap. Needed 3 RString(...) wraps for genuine reference-parameter hard boundaries instead: Attack::FromGlobalCourseModifier(const RString&) and PlayerOptions::FromString(const RString&) called from Trail.cpp (2 sites) and GameState.cpp (1 site). Course.cpp's te.Modifiers = e->sModifiers (RString assigned into std::string member) and CourseContentsList.cpp's msg.SetParam(...) needed no changes. Section-5-adjacent (course/trail loading) - re-verified both [corpus] (313/3) and [crs] (39/5) unchanged before/after. New lesson: a public struct member touched from multiple external files can still be a good pilot - it costs one wrap per hard-boundary call site, not per file. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0.
* **item 10 (RString retirement) scouting methodology bug, 2026-09-13 (autonomous, standing goal).** Attempted to migrate CommandLineActions::CommandLineArgs::argv (std::vector<RString>) after scouting external callers with `grep -rn ... src/*.cpp`, which found only one caller and looked clean. The shell glob src/*.cpp does not recurse into subdirectories, so it silently missed src/archutils/Win32/GraphicsWindow.cpp, which passes args.argv by reference into split(..., std::vector<RString>&, ...) - the exact container hard-boundary already known from the Command.cpp pilot. The full build (which compiles the whole tree, arch/archutils included) caught this immediately with a compile error before anything was committed, so no broken commit resulted - the safety net worked, but the scouting step gave a false "looks safe" signal. Reverted the attempted change (net zero diff, nothing to commit). New standing rule recorded in the playbook: always scout with a genuinely recursive search across all of src/ (no single-directory glob) since src/arch/ and src/archutils/Win32/ are real platform-code directories a src/*.cpp glob will never see.
* **item 9 (god-object split) phase 1, cluster 6, 2026-09-13.** Carved GameState's MultiPlayer-mode fields (m_MultiPlayerStatus, m_bMultiplayer, m_iNumMultiplayerNoteFields, m_pMultiPlayerState, IsMultiPlayerEnabled()) into a header-only GameStateMultiPlayerData.h, registered in CMakeData-singletons.cmake. First cluster that isn't a single contiguous block in the header - the fields sit at 3 separate spots interleaved with unrelated members and with the core 2-player fields (m_bSideIsJoined, m_pPlayerState) deliberately left alone as too foundational. Didn't need textual contiguity - each field became a reference member in its own original location, as long as the new component was declared before all of them. Scouted the real footprint with a genuinely recursive grep first (per the earlier methodology fix): ~34 sites across 9 external files, only GameState.cpp (18) and ScreenGameplay.cpp (8) had more than 1-2 touches. New kind of member handled: m_pMultiPlayerState is a heap-allocated pointer array (new PlayerState per slot in the constructor body, SAFE_DELETE in the destructor, both outside the init-list) - needed zero changes since new/delete/indexing pass through a reference-to-array transparently. IsMultiPlayerEnabled() moved as a real implementation; the free function GetNextEnabledMultiPlayer() calling GAMESTATE->IsMultiPlayerEnabled(mp) needed no changes either, since it goes through the still-identical public method. None of the four fields were in the original constructor init-list, so the new component has no explicit constructor. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 6 clusters done now (Edit, Workout, Attract, Autogen, Haste, MultiPlayer).
* **item 9 (god-object split) phase 1, cluster 7, 2026-09-13.** Carved GameState's "used by themes that support heart rate entry" fields (m_DanceStartTime a RageTimer, m_DanceDuration a float) into a header-only GameStateDanceData.h, registered in CMakeData-singletons.cmake. Pure data, no methods, tiny external footprint (only ScreenGameplay.cpp touches them via .Touch()/.Ago() and a plain read). Neither field was in GameState's original constructor init-list, but m_DanceStartTime being a class type (RageTimer) means it was always getting correctly default-constructed regardless (class-type members always default-construct unless explicitly initialized, unlike POD members) - the new component's own implicit default constructor reproduces this automatically. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 7 clusters done now (Edit, Workout, Attract, Autogen, Haste, MultiPlayer, Dance).
* **item 10 (RString retirement) pilot #11, 2026-09-13 (found while scouting item-9 clusters).** Migrated GameState::m_RandomAttacks (std::vector<RString> -> std::vector<std::string>) - a direct type change, not a god-object cluster (no reference-member technique needed since nothing external needs a reference bound to it). Only 2 external files (Player.cpp, SongManager.cpp), all via native container methods (.empty()/.size()/operator[]/.clear()/.push_back()) - no function takes the whole vector by reference to an un-migrated vector<RString>& parameter, unlike the Command.cpp/CommandLineActions container trap, so zero hard boundaries. Player.cpp's ApplyRandomAttack() returns RString from an operator[] read (safe implicit conversion); SongManager.cpp's push_back(sAttack) passes an RString local (safe upcast binding). Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0.
* **item 9 (god-object split) phase 1, cluster 8, 2026-09-13.** Carved GameState's PLAY_MODE_BATTLE/PLAY_MODE_RAVE fields (m_fOpponentHealthPercent, m_fTugLifePercentP1, both float) into a header-only GameStateBattleRaveData.h, registered in CMakeData-singletons.cmake. Pure data, no methods. Recursive grep across all 6 touching files (CombinedLifeMeterTug.cpp, Inventory.cpp, Player.cpp, ScoreKeeperRave.cpp, ScreenGameplay.cpp, GameState.cpp) found only plain reads/writes/comparisons, no hard boundaries. Neither field was in GameState's original constructor init-list, so the new component correctly has no explicit constructor - same shape as Haste/Dance, no new technique wrinkle. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 8 clusters done now (Edit, Workout, Attract, Autogen, Haste, MultiPlayer, Dance, BattleRave).
* **item 9 (god-object split) phase 1, cluster 9, 2026-09-13.** Carved GameState's per-game/round random seed fields (m_iGameSeed, m_iStageSeed both int, m_sStageGUID an RString left as RString - item-9 split only, no item-10 migration here) plus SetNewStageSeed() into a header-only GameStateStageSeedData.h, registered in CMakeData-singletons.cmake. SetNewStageSeed() moved as a real implementation since it only touches m_iStageSeed (rand() is a free function, no GameState singleton dependency). 5 external files (NoteDataUtil.cpp, StatsManager.cpp, Course.cpp, ScreenGameplay.cpp, ArrowEffects.cpp) all touch these fields via plain reads or the unchanged public SetNewStageSeed()/GAMESTATE-> accessors - no hard boundaries. NoteDataUtil.cpp/Course.cpp are section-5-adjacent - re-verified [corpus] (313/3) and [crs] (39/5) unchanged before/after. None of the three fields were in GameState's original constructor init-list, so the new component correctly has no explicit constructor. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 9 clusters done now (Edit, Workout, Attract, Autogen, Haste, MultiPlayer, Dance, BattleRave, StageSeed).
* **item 9 (god-object split) phase 1, cluster 10, 2026-09-13.** Carved GameState's "character stuff" field (m_pCurCharacters[NUM_PLAYERS], a Character* array) into a header-only GameStateCharacterData.h, registered in CMakeData-singletons.cmake. Array reference member, same shape as cluster 6's m_MultiPlayerStatus. Recursive grep across all 6 touching files (AttackDisplay.cpp, BeginnerHelper.cpp, DancingCharacters.cpp, GameCommand.cpp, ScoreKeeperRave.cpp, ScreenSelectCharacter.cpp) found only plain indexing/assignment/comparison, no hard boundaries, none section-5-adjacent. Field was not in GameState's original constructor init-list, so the new component correctly has no explicit constructor. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 10 clusters done now (Edit, Workout, Attract, Autogen, Haste, MultiPlayer, Dance, BattleRave, StageSeed, Character).
* **item 9 (god-object split) phase 1, cluster 11, 2026-09-13.** Carved GameState's private "Timing position corrections" fields (m_LastPositionTimer an RageTimer, m_LastPositionSeconds a float, m_paused a bool) into a header-only GameStatePositionCorrectionData.h, registered in CMakeData-singletons.cmake. Already private to GameState, so zero external exposure to begin with; touched only inside ResetMusicStatistics()/UpdateSongPosition() (both of which also touch unrelated GameState state so they stay on GameState, reading through the reference members transparently) plus the trivial inline SetPaused()/GetPaused(). None of the three fields were in GameState's original constructor init-list, so the new component correctly has no explicit constructor. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 11 clusters done now (Edit, Workout, Attract, Autogen, Haste, MultiPlayer, Dance, BattleRave, StageSeed, Character, PositionCorrection).
* **item 9 (god-object split) phase 1, cluster 12, 2026-09-13.** Carved GameState's MusicWheel expanded/last-open section fields (sExpandedSectionName, sLastOpenSection, both RString, contiguous in the header) into a header-only GameStateSectionData.h, registered in CMakeData-singletons.cmake. Only 2 external files touch the GAMESTATE-> fields directly (MusicWheelItem.cpp, MusicWheel.cpp) - WheelBase.cpp and MusicWheel.cpp also have their own same-named local member (m_sExpandedSectionName) on the wheel classes themselves, a different field entirely, not touched by this change. All usages are plain == comparisons or assignments, no hard boundaries. Neither field was in GameState's original constructor init-list, so the new component correctly has no explicit constructor. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 12 clusters done now (Edit, Workout, Attract, Autogen, Haste, MultiPlayer, Dance, BattleRave, StageSeed, Character, PositionCorrection, Section). Remaining GameState.h fields are either already-rejected coupled clusters or isolated single bools with no natural thematic grouping - item 9's low-risk phase-1 clusters are considered exhausted for now barring a maintainer call on how to handle the leftovers.
* **item 10 (RString retirement) pilot #12, 2026-09-13 (pivoted back to item 10 after item 9's low-risk clusters ran out).** Migrated PlayerStageStats::FormatPercentScore(float)'s own return type (RString -> std::string) - a static function's declared signature, not a member field. Body only uses ssprintf(...) (assigning its RString return into a std::string local is safe, slices the basic_string<char> base). Only 1 real external caller (PaneDisplay.cpp's sTextOut = FormatPercentScore(...), sTextOut being RString&) - safe because CStdStr has a genuine operator=(const std::string&) overload (StdString.h:391), not just the inherited base-class assignment. The LuaFunction(FormatPercentScore, ...) macro registration forwards through LuaHelpers::Push, already confirmed generic for both RString/std::string in pilot #10. Verified: sm_tests 5981/230 unchanged (rebuild recompiled only the ~40 files including PlayerStageStats.h, no wider cascade), ctest 100%, Release clean rebuild, --SelfTest exit 0. 12 RString pilots done now.
* **item 10 (RString retirement) pilot #13, 2026-09-13.** Migrated InputQueueCodeSet::Load(...)'s parameter and ::Input(...)'s return type (CodeSet.h/.cpp) from RString to std::string. m_asCodeNames (std::vector<RString>) stays RString - it feeds split(const RString&, const RString&, std::vector<RString>&, bool) (RageUtil.h:406), no vector<std::string>& overload exists, same container hard-boundary as CommandLineActions::argv/Command.cpp's m_vsArgs. Load's body calls THEME->GetMetric(const RString&, const RString&) through the CODE_NAMES/CODE(s) macros - fixed with 2 RString(sType) wraps inside the macro definitions themselves (localized to CodeSet.cpp). External callers (OptionsList.cpp's m_Codes.Load(sType), Screen.cpp's m_Codes.Load(m_sName)) both pass an RString local - safe, binds to const std::string& with zero changes. InputMessage's internal sCodeName local also switched to std::string (purely internal, feeds Message::SetParam, already confirmed generic in pilot #10). Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 13 RString pilots done now.
* **item 10 (RString retirement) pilot #14, 2026-09-13.** Migrated RandomSample's Load/LoadSoundDir/LoadSound parameters (RandomSample.h/.cpp) from by-value RString to by-value std::string. Fixed 5 hard-boundary calls into still-RString RageUtil free functions: GetExtension(const RString&) (1 wrap) and GetDirListing(const RString&, std::vector<RString>&, ...) (4 wraps, one per extension) - the output vector (arraySoundFiles) stays std::vector<RString> since GetDirListing fills it by reference. Replaced the RString-only .Right(1) with .substr(sDir.size()-1) per the playbook's mapping table. RageSound::Load(RString sFile) (by-value) needed no wrap - passing std::string into an RString by-value parameter is the established safe direction. Only 1 external caller (ScreenSelectMaster.cpp's m_soundDifficult.Load(ANNOUNCER->GetPathTo(...)), returning RString by value) needed zero changes. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 14 RString pilots done now.
* **item 10 (RString retirement) pilot #15, 2026-09-13.** Migrated RageWorkerThread's constructor parameter and m_sName member (RageUtil_WorkerThread.h/.cpp) from RString to std::string. Private member, no exposing getter, only used with .c_str() in LOG_TRACE/LOG_WARN. Zero wraps needed: the constructor body's "\"" + sName + "\" worker event" string arithmetic still produces std::string, which binds fine into RageEvent(RString name)'s by-value parameter (safe direction). Both external subclass callers (MemoryCardManager.cpp's string literal, RageFileDriverTimeout.cpp's ThreadedFileWorker forwarding its own by-value RString sPath) needed zero changes. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 15 RString pilots done now.
* **item 10 (RString retirement) pilot #16, 2026-09-13.** Migrated RageSurfaceUtils::SaveSurface/LoadSurface's by-value file parameter (RageSurfaceUtils.h/.cpp) from RString to std::string. RageFile::Open(const RString&, int) is a real reference-parameter hard boundary into the still-RString RageFile - fixed with 2 RString(file) wraps, one per function, both local to RageSurfaceUtils.cpp. Only 1 external caller file (ImageCache.cpp, 3 call sites), all passing a const RString sCachePath local - safe, binds to the by-value std::string parameter with zero changes. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 16 RString pilots done now.
* **item 10 (RString retirement) pilot #17, 2026-09-13 (found via a broad scouting fork).** Migrated FontManager::LoadFont(...)'s parameters (FontManager.h/.cpp) from RString to std::string. Font::Load(const RString&, RString) (Font.h:176) needed 1 RString(...) wrap on its first argument; the second (sChars) is by-value, no wrap needed. FontName (typedef std::pair<RString,RString>, the font cache's map key) needed no change - std::pair's templated constructor accepts std::string args converting implicitly to each RString field. Only 2 external callers, both in BitmapText.cpp, both passing const RString& locals - safe, zero changes. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 17 RString pilots done now. A dedicated scouting fork also confirmed backlog items #12/#15/#26/#29 are all genuinely maintainer-gated (clang-tidy not even installed for #12; live gameplay/editor reasoning needed for #15; #26 already fully worked through; #29 explicitly maintainer-deferred) rather than under-scouted.
* **item 9 (god-object split), Player/ScreenGameplay investigated and rejected, 2026-09-13.** Checked whether Player.cpp/ScreenGameplay.cpp (both listed in item 9's own oversized-file table) could take the same reference-member cluster treatment as GameState. Neither has GameState's crisp safety prerequisite (an already deleted/private copy constructor and assignment operator, confirmed before any GameState cluster work started). Player derives from Actor, which has a virtual Copy() clone method Player doesn't override - whether a Player is ever copied polymorphically is genuinely ambiguous, not provably impossible. Applying the reference-member technique to a copyable object would silently alias two objects' component storage - not pursued without either proof no copy path exists, or first adding an explicit deleted copy ctor/assignment as its own separate hardening change.
* **item 10 (RString retirement) pilot #18, 2026-09-13.** Migrated Inventory.cpp's file-local Item::sModifier field (struct Item is declared entirely inside the .cpp, not in any header - zero external exposure, the cleanest possible shape). Both touch points are safe-direction assignments already established this session: item.sModifier = ITEM_EFFECT(i) (RString-returning macro into std::string) and a.sModifiers = g_Items[...].sModifier (std::string into Attack::sModifiers, still RString, safe via CStdStr::operator=(const std::string&)). Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 18 RString pilots done now.
* **item 10 (RString retirement) pilot #19, 2026-09-13.** Migrated ProfileManager.cpp's file-local DirAndProfile::sDir field. Profile::LoadTypeFromDir/LoadAllFromDir/LoadEditableDataFromDir/SaveTypeToDir/HandleStatsPrefixChange (Profile.h:403-420) are all by-value RString parameters, so every call site needed zero changes. Two real hard boundaries: a local const RString &sOther = dap.sDir reference binding, fixed by changing its declared type to const std::string& (a derived-reference-to-base-object binding is simply invalid, so this needed a type change not a wrap); and two LocalProfileDirToID(const RString&, ...) calls, fixed with RString(...) wraps. struct DirAndProfile lives entirely in ProfileManager.cpp, never in a header - zero external exposure. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 19 RString pilots done now.
* **item 10 (RString retirement) pilot #20, 2026-09-13.** Migrated InputFilter.cpp's ButtonState::m_sComment field. ButtonState is only forward-declared in InputFilter.h and used solely as a reference parameter (CheckButtonChange) - its full definition and every touch of m_sComment lives entirely in InputFilter.cpp. The two real public entry points (GetButtonComment()/SetButtonComment(), InputFilter.h:74) keep their RString signatures unchanged since the internal conversions on both sides are already-established safe directions - zero external callers needed any change. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 20 RString pilots done now.
* **item 10 (RString retirement) pilot #21, 2026-09-13.** Migrated PaneDisplay.cpp's file-local Content_t::sFontType field. struct Content_t and its static const aggregate-initialized array (g_Contents[], string literals) both live entirely in the .cpp - zero external exposure. Only 1 touch point: RString sFontType = g_Contents[pc].sFontType - a local variable deliberately kept as RString, so this already just implicitly constructs an RString from the migrated field (safe direction), and the following THEME->GetPathF(...) call needed no wrap since it operates on the still-RString local, not the struct field. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 21 RString pilots done now.
* **item 10 (RString retirement) pilot #22, 2026-09-13.** Migrated RageFileManager.cpp's file-local LoadedDriver::m_sType/m_sRoot/m_sMountPoint fields - the biggest single-file pilot yet (27 touch points), fully mechanical and self-contained (LoadedDriver never named in RageFileManager.h). .CompareNoCase(...) is a CStdStr-only facade method with no direct std::string equivalent, but its own implementation is just ssicmp(this->c_str(), szThat) - StdString::ssicmp is a plain templated free function usable directly on two .c_str() results, so every .CompareNoCase(x) call became StdString::ssicmp(a.c_str(), x.c_str()) with byte-identical behavior (needed the StdString:: qualifier - first build attempt failed with C3861 until added). A local const RString &mountPoint reference binding needed retyping to const std::string& (same shape as pilot #19), cascading into a dependent local (trimPoint) needing the same fix. DriverLocation (a real public-API struct) was deliberately left as RString - assigning migrated fields into it is the established safe direction. Re-verified [corpus] (313/3) and [crs] (39/5) as extra precaution since this underlies all file I/O including song loading, even though LoadedDriver isn't part of the simfile parse path itself. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 22 RString pilots done now.
* **item 10 (RString retirement) pilots #23-24, 2026-09-13.** Migrated two more file-local structs found via the same technique: ScreenInstallOverlay.cpp's PlayAfterLaunchInfo::sSongDir/sTheme (zero hard-boundary fixes needed - every touch was already a safe direction, including SongManager::GetSongFromDir(RString) by-value), and StepMania.cpp's VideoCardDefaults::sDriverRegex/sVideoRenderers (needed 2 fixes: Preference<RString>::Set(const T&) wrapped with RString(...), and a .CompareNoCase(...) call fixed with the same StdString::ssicmp(a.c_str(), b.c_str()) substitution pilot #22 established). Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 24 RString pilots done now.
* **item 10 (RString retirement) pilot #25, 2026-09-13.** Migrated RageFileDriverMemory.cpp's file-local RageFileObjMemFile::m_sBuf field (an in-memory byte buffer used by RageFileObjMem). One real public API touch: GetString() previously returned const RString& - can't keep returning a reference once the field is std::string, so it changed to return RString by value instead, requiring zero caller changes anywhere (every caller either assigns into an RString/RString& or passes as a template argument that still deduces RString either way). PutString(const RString&) assigns safely into the std::string field. .replace()/operator[]/.size() are all inherited unmodified from std::basic_string in both RString and std::string, never RString-specific overrides, so behavior is byte-for-byte identical. Since this class is used by CourseWriterCRS.cpp (.crs course writing, a section-5 format) and XmlFileUtil.cpp/RageFileDriverDeflate.cpp (profile/stats serialization), re-verified both [crs] (39/5) and [corpus] (313/3) as extra precaution. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 25 RString pilots done now.
* **item 10 (RString retirement) pilot #26, 2026-09-13.** Migrated NoteSkinManager.cpp's NoteSkinData::sName field. struct NoteSkinData is forward-declared and used only by reference in NoteSkinManager.h - same shape as pilot #20's ButtonState. vsDirSearchOrder (vector<RString>) deliberately left untouched. Only 3 real touch points: a safe assignment, a safe push_back into a vector<RString>& out-param, and one hard boundary (LuaHelpers::ReportScriptError(RString const&, ...) receiving a concatenation expression that now evaluates to std::string) fixed with a single RString(...) wrap around just the sName term. Rejected in the same sweep: RageSoundReader_WAV.cpp's WavReader::m_sError (FileReading::read_8/read_16_le take a mutable RString& out-param used broadly elsewhere, no cheap fix) and XmlToLua.cpp's actor_template_t (8+ fields, a map member, 6+ methods, recursive vector - sized like its own dedicated pilot, deferred not rejected). Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 26 RString pilots done now.
* **item 10 (RString retirement) pilot #27, 2026-09-13.** Migrated ThemeManager.cpp's Theme::sThemeName and CompareLanguageTag::m_sLanguageString fields. Theme has an unused, otherwise-dead forward declaration in ThemeManager.h, never actually referenced there - its real definition and every usage live in the .cpp. CompareLanguageTag is fully file-local. Two const RString& hard boundaries for sThemeName (GetLanguagesForTheme/GetPathInfoToRaw) fixed with RString(...) wraps. m_sLanguageString.MakeLower() (an RString-only facade method) replaced with the guarded free-function form established in pilot #1, the very first pilot of this whole effort - confirming that lesson still applies unchanged 27 pilots later. Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 27 RString pilots done now.
* **item 10 (RString retirement) pilots #28-29, 2026-09-13 (found via a second scouting fork).** Migrated GameLoop.cpp's g_NewTheme/g_NewGame file-scope statics (5 hard-boundary wraps, all local to the file) and Style::ColToButtonName(int)'s return type (touched 3 files: Style.h/.cpp for the return type plus a lua_pushstring .c_str() fix, and GhostArrowRow.cpp/NoteDisplay.cpp which bound the result into const RString& - retyping those to const std::string& would have cascaded into 11+ more wraps in NoteDisplay.cpp alone, so switched to a by-value RString copy instead, which needed zero further changes downstream). Verified: sm_tests 5981/230 unchanged, ctest 100%, Release clean rebuild, --SelfTest exit 0. 29 RString pilots done now.
* **CI-caught fix, 2026-09-13.** Pilots #28-29's macOS CI jobs failed: GameLoop.cpp:183's `g_NewTheme = PREFSMAN->m_sTheme;` compiles on MSVC but not Clang - relies on Preference<T>::operator const T() (one user-defined conversion) producing an RString prvalue, then a derived-to-base slice into std::string::operator=, which libc++/Clang rejects as "no viable conversion from Preference<RString> to string" even though MSVC's STL accepts it. A genuine cross-compiler difference local Windows-only verification can't catch. Fixed with Preference<T>::Get() (returns const T& directly, no conversion-operator hop) instead: `g_NewTheme = PREFSMAN->m_sTheme.Get();`. Swept the rest of the session's pilots for the same PREFSMAN->m_s* direct-assignment shape - the other 3 hits all target still-RString variables, unaffected. New lesson: prefer Preference<T>::Get() over the bare implicit conversion whenever assigning a Preference<RString> into a std::string target. Verified locally (Windows): sm_tests 5981/230 unchanged, ctest 100%, Release rebuild, --SelfTest exit 0; pushed and awaiting macOS CI specifically to confirm.
