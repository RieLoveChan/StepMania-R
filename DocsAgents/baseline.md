...
- **Unit harness: MERGED** to `5_1-new` (2026-09-04,
  ...
  **Characterization files:**
  ...
  - `tests/test_NotesLoaderKSF.cpp` + `tests/data/Fixture Artist - KSF
    ...
    Suite total: **5759 assertions / 205 cases** (up from 311/72).
  ...
  - `tests/test_RageLog.cpp` (2026-09-08) — `RageLog`'s level/category
    ...
    unknown → Trace / General); `SetLogLevelSpec` parsing
    (`"warn"`, `"warn,gl:off,font:trace"`, per-cat only, bogus tokens)
  ...
  **Locally verified on Windows** (VS 2022 BuildTools cmake 3.31 — the
  ...
  - `WITH_TESTS=ON` Debug → `sm_engine` OBJECT lib + `Catch2` +
    ...
    → **5759 assertions / 205 cases pass**; `ctest` 100%. (First-landed at
    94/19.)
