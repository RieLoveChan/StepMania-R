...
static bool CodePageConvert(RString &sText, int iCodePage) {
	...
	iSize = MultiByteToWideChar(
	   iCodePage, MB_ERR_INVALID_CHARS, sText.data(), static_cast<int>(sText.size()), (wchar_t *)sOut.data(), iSize
	);
	ASSERT(iSize != 0);
...
bool ConvertString(RString &str, const RString &encodings) {
	...
	for (unsigned i = 0; i < lst.size(); ++i) {
		...
		RageException::Throw("Unexpected conversion string \"%s\" (string \"%s\").", lst[i].c_str(), str.c_str());
	}
