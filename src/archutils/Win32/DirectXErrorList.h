...
DXERRMSG(
   ...
   "Unable to IDirectInputJoyConfig_Acquire because the user does not have sufficient privileges to change the "
	"joystick configuration. & An invalid media type was specified"
)
...
DXERRMSG(
   DIERR_HASEFFECTS,
   "The device cannot be reinitialized because there are still effects attached to it. & At least one of the pins "
	"involved in the operation is already connected."
)
DXERRMSG(
   DIERR_NOTEXCLUSIVEACQUIRED,
   "The operation cannot be performed unless the device is acquired in DISCL_EXCLUSIVE mode. & This operation cannot "
	"be performed because the filter is active."
)
DXERRMSG(
   DIERR_INCOMPLETEEFFECT,
   "The effect could not be downloaded because essential information is missing.  For example, no axes have been "
	"associated with the effect, or no type-specific information has been created. & One of the specified pins supports "
	"no media types."
)
DXERRMSG(
   DIERR_NOTBUFFERED,
   "Attempted to read buffered device data from a device that is not buffered. & There is no common media type between "
	"these pins."
)
DXERRMSG(
   DIERR_EFFECTPLAYING,
   "An attempt was made to modify parameters of an effect while it is playing.  Not all hardware devices support "
	"altering the parameters of an effect while it is playing. & Two pins of the same direction cannot be connected "
	"together."
)
DXERRMSG(
   DIERR_UNPLUGGED,
   "The operation could not be completed because the device is not plugged in. & The operation cannot be performed "
	"because the pins are not connected."
)
DXERRMSG(
   DIERR_REPORTFULL,
   "SendDeviceData failed because more information was requested to be sent than can be sent to the device.  Some "
	"devices have restrictions on how much data can be sent to them.  (For example, there might be a limit on the "
	"number of buttons that can be pressed at once.) & No sample buffer allocator is available."
)
DXERRMSG(
   DIERR_MAPFILEFAIL,
   "A mapper file function failed because reading or writing the user or IHV settings file failed. & A run-time error "
	"occurred."
)
#endif
...
DXERRMSG(
   DIERR_DRIVERFIRST,
   "Device driver-specific codes. Unless the specific driver has been precisely identified, no meaning should be "
	"attributed to these values other than that the driver originated the error."
)
...
DXERRMSG(
   DIERR_CANCELLED,
   "The user cancelled the install operation. & The stream already has allocated samples and the surface doesn't match "
	"the sample format."
)
DXERRMSG(
   DIERR_BADINF,
   "The INF file for the selected device could not be found or is invalid or is damaged. & The specified purpose ID "
	"can't be used for the call."
)
#endif
...
DXERRMSG(
   ...
   "The application has made an erroneous API call that it had enough information to avoid. This error is intended to "
	"denote that the application should be altered to avoid the error. Use of the debug version of the DXGI.DLL will "
	"provide run-time debug output with further information."
)
DXERRMSG(
   DXGI_ERROR_NOT_FOUND,
   "The item requested was not found. For GetPrivateData calls, this means that the specified GUID had not been "
	"previously associated with the object."
)
...
DXERRMSG(
   ...
   "The application attempted to perform an operation on an DXGI output that is only legal after the output has been "
	"claimed for exclusive owenership."
)
DXERRMSG(
