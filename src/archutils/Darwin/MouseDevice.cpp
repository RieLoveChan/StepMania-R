...
Mouse::Mouse()
    : id(InputDevice_Invalid), x_axis(0), y_axis(0), z_axis(0), x_min(0), x_max(0), y_min(0), y_max(0), z_min(0),
      z_max(0) {
}
...
#define ADD(x)                                                                                                         \
	if (m.x)                                                                                                            \
	AddElementToQueue(m.x)
	ADD(x_axis);
	ADD(y_axis);
	ADD(z_axis);
#undef ADD
	for (std::unordered_map<IOHIDElementCookie, DeviceButton>::const_iterator i = m_Mapping.begin();
	     i != m_Mapping.end();
	     ++i)
