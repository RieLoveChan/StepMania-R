...
#define FOREACH_Attr(pNode, Var)                                                                                       \
	for (XAttrs::iterator Var = (pNode)->m_attrs.begin(); Var != (pNode)->m_attrs.end(); ++Var)
/** @brief Loop through each node, using a constant iterator. */
#define FOREACH_CONST_Attr(pNode, Var)                                                                                 \
	for (XAttrs::const_iterator Var = (pNode)->m_attrs.begin(); Var != (pNode)->m_attrs.end(); ++Var)
/** @brief Loop through each child. */
#define FOREACH_Child(pNode, Var)                                                                                      \
	XNode *Var = nullptr;                                                                                               \
	for (XNodes::iterator Var##Iter = (pNode)->GetChildrenBegin();                                                      \
	     Var = (Var##Iter != (pNode)->GetChildrenEnd()) ? *Var##Iter : nullptr, Var##Iter != (pNode)->GetChildrenEnd(); \
	     ++Var##Iter)
...
class XNode {
 ...
 public:
	...
	template <typename T> void GetTextValue(T &out) const {
	...
	XNodes::const_iterator GetChildrenBegin() const {
		return m_childs.begin();
	}
	XNodes::iterator GetChildrenEnd() {
		return m_childs.end();
	}
	XNodes::const_iterator GetChildrenEnd() const {
		return m_childs.end();
	}
