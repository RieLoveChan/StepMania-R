...
#define PAM_DEPTH(p)                                                                                                   \
	PAM_ASSIGN(                                                                                                         \
	   (p),                                                                                                             \
	   (std::uint8_t)table[PAM_GETR(p)],                                                                                \
	   (std::uint8_t)table[PAM_GETG(p)],                                                                                \
	   (std::uint8_t)table[PAM_GETB(p)],                                                                                \
	   (std::uint8_t)table[PAM_GETA(p)]                                                                                 \
	)
...
void RageSurfaceUtils::Palettize(RageSurface *&pImg, int iColors, bool bDither) {
	...
	RageSurfaceUtils::ConvertSurface(
	   pImg, pImg->w, pImg->h, 32, Swap32BE(0xFF000000), Swap32BE(0x00FF0000), Swap32BE(0x0000FF00), Swap32BE(0x000000FF)
	);
	...
	// Rescale the palette colors to a maxval of 255.
	{
		...
		for (int x = 0; x < pal->ncolors; ++x) {
			...
			pal->colors[x].r = (PAM_GETR(acolormap[x].acolor) * 255 + (maxval >> 1)) / maxval;
			pal->colors[x].g = (PAM_GETG(acolormap[x].acolor) * 255 + (maxval >> 1)) / maxval;
			pal->colors[x].b = (PAM_GETB(acolormap[x].acolor) * 255 + (maxval >> 1)) / maxval;
			pal->colors[x].a = (PAM_GETA(acolormap[x].acolor) * 255 + (maxval >> 1)) / maxval;
		}
	}
...
#define pam_hashapixel(p)                                                                                              \
	(((unsigned)PAM_GETR(p) * 33023 + (unsigned)PAM_GETG(p) * 30013 + (unsigned)PAM_GETB(p) * 27011 +                   \
